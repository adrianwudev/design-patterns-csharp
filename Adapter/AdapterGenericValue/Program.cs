...
namespace AdapterGenericValue
{
        ...
        where TSelf : Vector<TSelf, T, D>, new()
    {
        ...
        {
            var requireSize = new D().Value;
            data = new T[requireSize];

            var providedSize = values.Length;

            for (int i = 0; i < Math.Min(requireSize, providedSize); i++)
                data[i] = values[i];
        }
    ...
    public class Vecotr3f 
        : VectorOfFloat<Vecotr3f, Dimensions.Three>
    {
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Vecotr3f u = Vecotr3f.Create(3.5f, 2.2f, 1);
        }
    }
}
